...
int sign_it(const byte *msg, size_t mlen, byte **sig, size_t *slen, EVP_PKEY *pkey)
{
    ...
    *slen = 0;
    ...
    do
    {
        ctx = FIPS_md_ctx_create();
        assert(ctx != NULL);
        if (ctx == NULL)
        {
            printf("FIPS_md_ctx_create failed, error 0x%lx\n", ERR_get_error());
            break; /* failed */
        }

        const EVP_MD *md = FIPS_get_digestbynid(getIdByName(hn));
        assert(md != NULL);
        if (md == NULL)
        {
            printf("FIPS_get_digestbynid failed, error 0x%lx\n", ERR_get_error());
            break; /* failed */
        }

        int rc = FIPS_digestinit(ctx, md);
        assert(rc == 1);
        if (rc != 1)
        {
            printf("FIPS_digestinit failed, error 0x%lx\n", ERR_get_error());
            break; /* failed */
        }
        ...
        rc = FIPS_digestupdate(ctx, msg, mlen);
        assert(rc == 1);
        if (rc != 1)
        {
            printf("FIPS_digestupdate failed, error 0x%lx\n", ERR_get_error());
            break; /* failed */
        }
...
int make_keys(EVP_PKEY **skey, EVP_PKEY **vkey)
{
    ...
    do
    {
        ...
        /* Generate bytes */
        int rc = FIPS_rand_bytes(hkey, size);
        assert(rc == 1);
        if (rc != 1)
        {
            printf("FIPS_rand_bytes failed, error 0x%lx\n", ERR_get_error());
            break;
        }
